 ...
 //The core supports zero-wait state accesses on all transfers.
  //It is allowed to drive APB_PREADY with a hard wired signal
	...
	begin
		...
		if(in[l] === 1'bx ) begin //<-------if(in[l] != 1'b1 && in[l] != 1'b0 )
  ...
  always @(posedge APB_CLK,negedge APB_RESET_n) 
	  ...
	  if (APB_PSEL & APB_PENABLE & APB_PWRITE & (APB_PADDR == DIRECTION)) begin
			...
			APB_PREADY <= 1'b1 ; //after successful write to dir_reg set PREADY = 1 
		end	
	 end
  ...
  //treat writes to Input register same	
    ...
    if      (!APB_RESET_n ) out_reg <= {PDATA_SIZE{1'b0}};
      ...
      if(APB_PREADY) begin
			...
			else //if not ready then drive out_reg with z
				out_reg<= 32'bz	;
		end	
    ...
    case (APB_PADDR)
      ...
      INPUT    :
        if((APB_PSEL & APB_PENABLE & ~APB_PWRITE  )) begin
			APB_PRDATA <= APB_PREADY? in_reg : 32'bz; 
		end
